...
namespace LofiUI.Texts
{
    ...
    public class TextArea : Control
    {
        ...
        {
            GraphicsManager.DrawT(Texture, new Rectangle(AbsLeft, AbsTop, Width, Height));

            int addY = 0;
            foreach (String line in lines)
            {
                GraphicsManager.DrawLine(line, new Vector2(AbsLeft + MarginLeft, AbsTop + addY));
                addY += LineSpacing;
            }
        }
        #endregion
        ...
        private void onTextModeChangeHandler(Object sender, EventArgs e)
        {
            ...
            {
                int curLen = 1;
                while (GraphicsManager.getTextSize(t.Substring(0, curLen)).X < lineWidth)
                {
                    curLen++;
