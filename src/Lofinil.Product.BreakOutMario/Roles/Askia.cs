...
namespace BreakOutMario.Roles
{
    ...
    public class Askia : Role
    {
        ...
        public Askia()
        {
            ...
            GetTween("Wonder").OnStart += Wonder_OnStart;
            GetTween("Wonder").OnFinish += Wonder_OnFinish;
            paramTweenList.Add(new RelaRateModifier("Dash", "Position"));
            GetTween("Dash").OnStart += Dash_OnStart;
            GetTween("Dash").OnFinish += Wonder_OnFinish;
            paramTweenList.Add(new RelaPosModifier("TakeOff", "Position"));
            GetTween("TakeOff").OnStart += Dash_OnStart;
            GetTween("TakeOff").OnFinish += Wonder_OnFinish;
        ...
        public void Wonder_OnStart(Object sender, EventArgs e)
        {
            ...
            if (rand == 0)
                GetTween("Wonder").EndValue = new Vector2(WonderLeft, WonderHeight).ToString();
            else
                GetTween("Wonder").EndValue = new Vector2(WonderRight, WonderHeight).ToString();
        }
        ...
        {
            GetTween("Dash").StartValue= Position.ToString();
            Vector2 speedVec = SceneManager.Instance.GetItemByName("Role").Position - Position;
            speedVec.Normalize();
            speedVec *= DashSpeed;
            GetTween("Dash").EndValue = speedVec.ToString();
        }
        ...
        {
            GetTween("TakeOff").EndValue = new Vector2(Position.X, WonderHeight).ToString();
        }
        ...
        public override bool Update()
        {
            ...
            // FSM更新
            if (SceneManager.Instance.GetItemByName("Role") != null)
            {
                FSMUpdate();
            }
            return true;
        ...
        {
            if (SceneManager.Instance.GetItemByName("Role").Position.X < (Position.X - Size.X))
            {
                FaceRight = true;
            }
            if (SceneManager.Instance.GetItemByName("Role").Position.X > (Position.X + Size.X))
        ...
        private void FSMUpdate()
        {
            switch (AskiaState)
            {
                ...
                case EAskiaState.Dashing:
                    #region 转出Dashing
                    if (GetTween("Dash").Tweening == false)
                    {
                        ChangeStateAction(EAskiaState.TakingOff);
                    }
                    #endregion
                    #region 转出LostControl
                    if (SceneManager.Instance.GTMActive)
                    {
                        StopTween("Dash");
                ...
                case EAskiaState.LostControl:
                    ...
                    #endregion
                    #region 转出GetHurt
                    foreach (Thorn t in SceneManager.Instance.GetItemManagerByName("Thorn").ItemList)
                    {
                        if (t.ThornState == Thorn.EThornState.Ice)
                        {
                            if (GetOrthoBound().Intersects(t.GetOrthoBound()))
                            {
                ...
                case EAskiaState.Wondering:
                    FaceControl();
                    #region 转出ShootingThorn/Dashing/Wondering/Stop
                    if (GetTween("Wonder").Tweening == false)
                    {
                        ChangeStateAction(ThinkNextAction());
                    }
                    #endregion
                ...
                case EAskiaState.TakingOff:
                    FaceControl();
                    #region 转出ShootingThorn/Dashing/Wondering/Stop
                    if (GetTween("TakeOff").Tweening == false)
                    {
                        ChangeStateAction(ThinkNextAction());
                    }
                    #endregion
        ...
        private EAskiaState ThinkNextAction()
        {
            ...
            while (!thinkStateValid)
            {
                ...
                {
                    // 是否可以冲刺
                    if (Math.Abs(SceneManager.Instance.GetItemByName("Role").Position.X - Position.X) > MinDashDest)
                    {
        ...
        private void ThrowThorn()
        {
            ...
            if (!SceneManager.Instance.ItemManagers.ContainsKey("Thorn"))
                SceneManager.Instance.ItemManagers.Add("Thorn", new ItemManager());
            SceneManager.Instance.ItemManagers["Thorn"].Add(thorn);
        }
        #endregion
